...
\noindent If $n \geq k$, there are infinitely many solutions that maximize the likelihood.  If $k < n$, there is a unique solution to this problem given by
...
\mathbb{V}_{\beta} (\widehat{\beta}_{\textrm{ML}}) = \mathbb{E}_{\mathbb{P}_{\beta}} [ \widehat{\beta}_{\textrm{ML}} \widehat{\beta}_{\textrm{ML}} ‘  ]  = (X’X)^{-1} X’ \mathbb{E}_{\mathbb{P}_{\beta}} [ Y Y’] X (X’X)^{-1} = \sigma^2 (X’X)^{-1}.  
\end{equation}
